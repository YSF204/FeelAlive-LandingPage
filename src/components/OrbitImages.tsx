...
function OrbitItem({ item, index, totalItems, pathValue, itemSizeValue, rotationValue, progress, fill, scaleStrength, focalPoint = 50 }) {
  ...
  const itemScale = useTransform(() => {
    ...
    } else targetScale = 0.4
    return 1 - strength * (1 - targetScale)
  })
  const offsetPath = useMotionTemplate`path("${pathValue}")`

  return (
    <motion.div className="orbit-item" style={{ width: itemSizeValue, height: itemSizeValue, offsetPath, offsetRotate: '0deg', offsetAnchor: 'center center', offsetDistance, scale: itemScale, zIndex: useTransform(itemScale, (s) => Math.round(s * 100)), pointerEvents: 'auto' }}>
      <motion.div style={{ transform: useTransform(rotationValue, (r) => `rotate(${-r}deg)`), width: '100%', height: '100%' }}>{item}</motion.div>
    </motion.div>
  )
}

export default function OrbitImages({ images = [], altPrefix = 'Orbiting image', shape = 'ellipse', customPath, baseWidth = 1400, radiusX = 700, radiusY = 170, radius = 300, starPoints = 5, starInnerRatio = 0.5, rotation = -8, duration = 40, itemSize = 64, direction = 'normal', fill = true, width = 100, height = 100, className = '', showPath = false, pathColor = 'rgba(0,0,0,0.1)', pathWidth = 2, easing = 'linear', paused = false, centerContent, responsive = false, progressOverride, radiusXOverride, radiusYOverride, itemSizeOverride, rotationOverride, translateXOverride, focusStrength }: any) {
  const containerRef = useRef(null)
  const [scale, setScale] = useState(1)
  const designCenterX = baseWidth / 2
  const designCenterY = baseWidth / 2
  const defaultRadiusX = useMotionValue(radiusX)
  const defaultRadiusY = useMotionValue(radiusY)
  const defaultItemSize = useMotionValue(itemSize)
  const defaultRotation = useMotionValue(rotation)
  const defaultTranslateX = useMotionValue(0)
  const currentRadiusX = radiusXOverride || defaultRadiusX
  const currentRadiusY = radiusYOverride || defaultRadiusY
  const currentItemSize = itemSizeOverride || defaultItemSize
  const currentRotation = rotationOverride || defaultRotation
  const currentTranslateX = translateXOverride || defaultTranslateX
  ...
  useEffect(() => {
    ...
    const updateScale = () => containerRef.current && setScale(containerRef.current.clientWidth / baseWidth)
    updateScale()
  ...
  useEffect(() => {
    ...
    const controls = animate(internalProgress, direction === 'reverse' ? -100 : 100, { duration, ease: easing, repeat: Infinity, repeatType: 'loop' })
  ...
  const items = useMemo(() => images.map((image, index) => (
    <motion.figure key={image.src} className="orbit-card" whileHover={{ scale: 1.04 }} transition={{ duration: 0.3 }}>
      <div className="orbit-image-frame">
        <img src={image.src} alt={`${altPrefix}: ${image.name}`} draggable={false} className="orbit-image" />
      </div>
      <figcaption className="orbit-caption">
        <span className="orbit-caption-number">{image.number}</span>
        <span className="orbit-caption-name">{image.name}</span>
      </figcaption>
    </motion.figure>
  )), [images, altPrefix])

  return (
    <div ref={containerRef} className={`orbit-container ${className}`} style={{ width: containerWidth, height: containerHeight, aspectRatio: responsive ? '1 / 1' : undefined }} aria-label="Feel Alive botanical collection">
      <div className={responsive ? 'orbit-scaling-container orbit-scaling-container--responsive' : 'orbit-scaling-container'} style={{ width: responsive ? baseWidth : '100%', height: responsive ? baseWidth : '100%', transform: responsive ? `translate(-50%, -50%) scale(${scale})` : undefined }}>
        <motion.div className="orbit-rotation-wrapper" style={{ rotate: currentRotation, x: currentTranslateX }}>
          {showPath && <svg width="100%" height="100%" viewBox={`0 0 ${baseWidth} ${baseWidth}`} className="orbit-path-svg"><motion.path d={pathValue} fill="none" stroke={pathColor} strokeWidth={pathWidth / scale} /></svg>}
          {items.map((item, index) => <OrbitItem key={index} item={item} index={index} totalItems={items.length} pathValue={pathValue} itemSizeValue={currentItemSize} rotationValue={currentRotation} progress={activeProgress} fill={fill} scaleStrength={focusStrength} focalPoint={50} />)}
        </motion.div>
      </div>
